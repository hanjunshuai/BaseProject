...
public abstract class GroupedRecyclerViewAdapter<D extends SectionEntity> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private OnLongHeaderClickListener mOnLongHeaderClickListener;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(getLayoutId(mTempPosition, viewType), parent, false);
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        ...
        if (type == TYPE_HEADER) {
            ...
            if (mOnLongHeaderClickListener != null) {
                holder.itemView.setOnLongClickListener(view -> {
                    mOnLongHeaderClickListener.onLongHeaderClick(GroupedRecyclerViewAdapter.this,
                            (BaseViewHolder) holder, groupPosition);
                    return true;
                });
            }
            ...
            if (mOnLongChildClickListener != null) {
                holder.itemView.setOnLongClickListener(view -> {
                    mOnLongChildClickListener.onLongChildClick(GroupedRecyclerViewAdapter.this,
                            (BaseViewHolder) holder, groupPosition, childPosition);
                    return true;
                });
            }
    ...
    public void setOnLongHeaderClickListener(OnLongHeaderClickListener onLongHeaderClickListener) {
        mOnLongHeaderClickListener = onLongHeaderClickListener;
    }
    ...
    /**
     * 返回头部的布局 id （如果hasHeader返回false，这个方法不会执行）
     *
     * @param viewType
     * @return
     */
    ...
    /**
     * 返回尾部的布局 id （如果hasFooter返回false, 这个方法不会执行）
     *
     * @param viewType
     * @return
     */
    ...
    public boolean isExpand(int groupPosition) {
        SectionEntity entity = mData.get(groupPosition);
